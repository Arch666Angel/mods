...
for _, prototype_type in pairs{
  ...
  for _, wagon_name in pairs(cargo_wagons) do
    local additional_pastable_entities = data.raw["cargo-wagon"][wagon_name].additional_pastable_entities or {}
    for prototype_name, _ in pairs(data.raw[prototype_type]) do
      table.insert(additional_pastable_entities, prototype_name)
    end
    data.raw["cargo-wagon"][wagon_name].additional_pastable_entities = additional_pastable_entities
  end
end