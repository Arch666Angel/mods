...
local on_entity_settings_pasted = function(event)
  ...
  if (wagonsToCopy[source.name] or wagonsToCopy[destination.name])                    -- correct train to do behaviour for
    and (sourceEntityTypes[source.type] and destinationEntityTypes[destination.type]  -- paste to a cargo or fluid wagon
    or   sourceEntityTypes[destination.type] and destinationEntityTypes[source.type]) -- copy from a cargo or fluid wagon
  then
    ...
    if source_color then
      ...
      log(serpent.block(destination.color))
      destination.color = {
        r = source_color.r,
        g = source_color.g,
        b = source_color.b,
