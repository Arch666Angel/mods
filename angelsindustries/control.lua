    ...
    if surface and surface.valid then
      ...
      if crash_site_entity and crash_site_entity.valid then
        local created_entity =
          ...
          name = main_lab[0],
          position = surface.find_non_colliding_position --[[name]](
            main_lab[0] --[[center]],
            {
        ...
        if created_entity and created_entity.valid then
          ...
          for k = 1, 3 do
            ...
            explosions.time_to_live =
              math.random(60 * 20, 60 * 30) - math.min((8 + (math.random() * 40)) * 100, 15 * 60)
...
script.on_event(
  ...
  function(event)
    ...
    if force then
      local available =
        force.technologies["angels-hidden-ghosting"] and
        force.technologies["angels-hidden-ghosting"].researched or
        false
    ...
    if ghosting[research.name] then
      ...
      if ghosting[research.name] then
        ...
        for tech, _ in pairs(ghosting) do
          if
            research.force.technologies[tech] and (not research.force.technologies[tech].researched)
           then
...
-- toggle ghosting
script.on_event(
  {defines.events.on_lua_shortcut, "toggle-ghosting"},
  function(event)
...
script.on_configuration_changed(
  function(event)
    if global.vehicle_burners == nil then
      global.vehicle_burners = {}
    end
    if global.current_car == nil then
      global.current_car = {}
    end
  end
)
...
script.on_event(
  ...
  function(event)
    if
      event.gui_type == defines.gui_type.entity and event.entity ~= nil and
...
script.on_event(
  ...
  function(event)
    if
      event.gui_type == defines.gui_type.entity and event.entity.type == "car" and
        global.current_car == event.entity
     then
      global.current_car = {}
    end
          ...
          if burner.currently_burning ~= nil then
            if car_content[burner.currently_burning.name] ~= nil then
              ...
              local removed =
                car.remove(
                {
                  name = burner.currently_burning.name,
                  count = amount
                }
              )
              burner.inventory.insert(
                {
                  name = burner.currently_burning.name,
                  count = removed
                }
              ...
              for i, item in pairs(car_content) do
                if
                  game.item_prototypes[i] ~= nil and
                    burner.fuel_categories[game.item_prototypes[i].fuel_category] ~= nil and
                    game.item_prototypes[i].fuel_value ~= nil
                 then
                  if
                    highest_fuel ~= nil and i ~= highest_fuel and
                      game.item_prototypes[i].fuel_value >
                        game.item_prototypes[highest_fuel].fuel_value
                   then
                    highest_fuel = i
