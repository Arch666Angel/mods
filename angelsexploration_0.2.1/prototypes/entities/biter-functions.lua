...
function make_run_animation(data_run)
...
function make_attack_animation(data_attack)
...
function make_die_animation(data_die)
...
function make_roar_sound(type, volume)
...
function make_die_sound(type, volume)
	if type == "biter" or type == "scarab" then
	  return
	  {
		{
...
function make_call_sounds(volume)
  return
  {
    {
...
function make_projectile(pro_app, pro_dmg)
data:extend(
{
  {
    type = "beam",
...
function make_attack_parameter(data_app, data_dmg)
...
function make_alien_spawner(spawn_data)
data:extend(
{
  {
    type = "unit-spawner",
...
function update_spawner(us_data)
	local s_name = us_data.appearance.name.."-"..us_data.appearance.type
	if data.raw.unit.s_name then
		local spawner = data.raw.spawner.s_name
		spawner.resistance = us_data.appearance.resistance
