    ...
    for _, tech_effect in pairs(tech_prototype.effects) do
      if (tech_effect.type == "unlock-recipe") and (not recipes_to_ignore[tech_effect.recipe]) then
        ...
        if recipe_prototype.enabled then
          unit_test_functions.print_msg(string.format("Tech %q unlocks recipe %q which is enabled at the beginning of the game.", tech_name, tech_effect.recipe))
          unit_test_result = unit_test_functions.test_failed -- soft failure
        end
        if recipe_prototype.hidden then
          unit_test_functions.print_msg(string.format("Tech %q unlocks recipe %q which is hidden.", tech_name, tech_effect.recipe))
          unit_test_result = unit_test_functions.test_failed -- soft failure
        end
      end
    end
