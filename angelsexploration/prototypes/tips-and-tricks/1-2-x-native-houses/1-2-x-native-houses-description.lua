...
return function(spawner_name, spawn_data)
  ...
  table.insert(description, {"tips-and-tricks-description.angels-native-inhabitants-spawner-intro", spawner_name, tnt.number_to_string(first_evolution_factor*100,2).."%"})
  ...
  for unit_idx, unit_data in pairs(spawn_data) do
    ...
    if unit then
      ...
      table.insert(spawn_unit, unit.localised_name or {"tips-and-tricks-name.angels-native-inhabitants-unit", "__ENTITY__"..unit_name.."__"})
  ...
  if loot and (#loot>0) then
    ...
    for _,drop in pairs(loot) do
      ...
      if prob == "100" then
        if min == max then
          table.insert(drop_description, {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-amount", min})
        else
          table.insert(drop_description, {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-amount-range", min, max})
        end
      else
        if min == max then
          if max == "1" then
            table.insert(drop_description, {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-percentage", prob})
          else
            local drop_combo = {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-percentage-amount-combo"}
            table.insert(drop_combo, {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-percentage", prob})
            table.insert(drop_combo, {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-amount", max})
            table.insert(drop_description, drop_combo)
          end
        else
          local drop_combo = {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-percentage-amount-combo"}
          table.insert(drop_combo, {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-percentage", prob})
          table.insert(drop_combo, {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-amount-range", min, max})
          table.insert(drop_description, drop_combo)
        end
      end
